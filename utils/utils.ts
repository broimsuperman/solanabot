...
export async function sendDiagnostics(data: string) {
  try {
    // Create a transporter with proper SMTP configuration
    ...
    const mailOptions = {
      from: '', // Replace with your sender's email
    ...
    console.log('Diagnostics email sent successfully');
  } catch (error) {
    console.error('Failed to send diagnostics email:' + error);
  }
}
